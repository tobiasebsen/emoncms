...
class Feed
{
    ...
    public function create($userid,$name,$datatype,$prop)
    {
        ...
        $result = $this->mysqli->query("INSERT INTO feeds (userid,name,datatype,public,`interval`) VALUES ('$userid','$name','$datatype','false','$prop')");
        ...
        if ($feedid>0) 
        {
          ...
          if ($datatype==0) {
            ...
            if ($prop<5) $prop = 5; 
            $this->timestore->create_node($feedid,$prop);
          } elseif ($datatype==1) {
            // CREATE HISTOGRAM FEED									
  ...
  */
  ...
  public function get_user_feeds($userid)
  {
    ...
    $result = $this->mysqli->query("SELECT id,name,datatype,tag,time,value,public,`interval` FROM feeds WHERE userid = $userid");
    if (!$result) return 0;
    $feeds = array();
